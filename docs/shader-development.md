...
Creator 3 的 `passIndex` 对应查询结果的 `rows[].pass`（当前启用的 Pass），`effectPass` 是 Effect technique 中的原始编号，`propertyIndex` 是共享属性编号。关闭的描边等可选 Pass 会使两种索引不同；持久化修改会映射到原始编号。Pass 的 `switch` 名称也作为布尔宏校验。

FBX 的 `.material` 子资源可通过 `material.query/properties/defines/states` 读取，查询返回 `readonly: true`。读取使用 AssetDB 与引擎序列化，不直接编辑导入缓存；需要修改时先用 `material.clone` 提取为工程内独立 `.mtl`，再按资源哈希更新。
