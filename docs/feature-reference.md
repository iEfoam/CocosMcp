...
Creator 3 的 `prefab.apply` 返回 `nativeResult`、`resourceChanged` 与前后资源哈希。3.8.8 原生接口可能返回 `false` 但文件已经写回，不能仅凭布尔值判断保存结果。`asset-write-observed` 只表示观察到资源内容变化，不等于视觉验收；未获原生确认且内容未变化时返回 `OUTCOME_UNKNOWN`，应先检查实例覆盖项再决定是否重试。
