...
本文保留 **2026-09-16 Creator 3.8.8** 的批次记录，175 项测试和构建标识是当时数据。2026-09-18 最近代码回归为 249 项；2.4.15 后续独立适配见 [版本矩阵](version-support.md) 和 [验收台账](creator2-expansion-tracker.md)，不能合并不同版本的原生认证。
...
**本 3.8.8 批次尚无真实 Spine/TMX 资产专项验收，也未完成所有 13 模板的完整玩法测试。** 2.4.15 后续样例验收不改变这里的 3.8.8 结论。 接触事件采样已有断连清理测试，尚未证明各后端的接触事件序列。Windows、其他 Creator 版本、原生设备、小游戏、WebGL/GPU 性能另行验收。本批验收未证明商店发布或生产部署；提交和发布状态须另查 Git/发布记录。
