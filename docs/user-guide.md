...
底部显示运行中的扩展版本和构建标识。“更新版本”从安装器记录的本地构建目录安装，自动备份旧扩展；同一版本号下不同构建也能更新。请先在源码工程执行构建，再点击更新。当前不提供 GitHub 在线下载更新。

文件安装成功后会显示“待重载”，正在运行的版本保持原值。在扩展管理器中重载后，新版本才生效；重载会停止扩展管理的 MCP 服务，需要重新启动。
