...
更新日期：2026-09-18。功能统计见 [版本矩阵](version-support.md)，复现见 [适配记录](creator2-implementation.md)。下表是当前状态，后半保留历史增量；早期“待补”以当前表格和较新条目为准。
...
| 03 | 场景与 Prefab 审计 | 场景/事件引用、保存文件 UUID 审计及删除守卫通过；Prefab 差异/修复待补 | 实例差异、断引、事件、重复组件、带计划修复 |
...
| 11 | Camera/RenderTexture | 2D 状态、2D/3D 坐标及局部视口、Graphics/Mask 像素与临时 GPU 删除通过；3D 模型像素/一般遮挡待补 | 坐标、可见性、离屏像素与临时资源释放 |
...
| 15 | 单步执行 | 暂停超时诊断通过；单步与调度待探针验证 | 暂停、帧推进、Scheduler/动画/物理时序 |
