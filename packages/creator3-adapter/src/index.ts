...
import { PrefabService } from './prefab.js';
  ...
  private async executePrepared(id: string, p: JsonObject): Promise<JsonValue> {
    ...
    switch (id) {
      ...
      case 'scene.open': {
        ...
        if (info.type !== 'cc.SceneAsset' || typeof info.uuid !== 'string' || info.invalid === true) throw new CocosError('INVALID_ARGUMENT', 'Expected a valid SceneAsset');
        await this.ensureSaved();
        await this.scene('open-scene', info.uuid);
        const result = Json.object(await this.execute('scene.query', {}));
        if (Json.object(result.scene).sceneId !== info.uuid) throw new CocosError('VERIFICATION_FAILED', 'Editor did not open the requested scene', { expectedSceneUuid: info.uuid, actual: result });
        return result;
      }
      ...
      case 'prefab.create': {
        ...
        const expectedRootName = basename(str('url'), '.prefab');
        // Creator 会按新预制体文件名重命名根节点；只允许这一项已知变化，子节点路径和名称仍须完全对应。
        if (!assetUuid || oldRows.length !== newRows.length || oldRows.some((row, index) => row.path !== newRows[index]!.path ||
          (row.name !== newRows[index]!.name && !(index === 0 && row.path === '' && newRows[index]!.name === expectedRootName)))) {
        ...
        return { assetUuid, rootId, ...(oldRows[0]?.name !== newRows[0]?.name ? { previousRootName: oldRows[0]!.name!, rootName: newRows[0]!.name! } : {}), rows: oldRows.map((row, index) => ({ path: row.path!, previousNodeId: row.nodeId!, nodeId: newRows[index]!.nodeId! })) };
      }
      case 'prefab.apply': return new PrefabService(this.port).apply(str('nodeId'));
