...
export class MaterialController {
  ...
  describe(material: RuntimeObject): JsonObject {
    return { materialUuid: A.uuid(material), effectUuid: material.effectAsset ? A.uuid(material.effectAsset) : null, technique: Number(material.technique ?? 0),
      rows: this.passes(material).map((pass, index) => ({ pass: index, effectPass: Number(pass.passIndex ?? index), propertyIndex: Number(pass.propertyIndex ?? pass.passIndex ?? index), properties: A.safeData(pass.properties), defines: A.safeData(pass.defines),
  ...
  private validateDefines(material: RuntimeObject, defines: JsonObject): void {
    ...
    // Pass switch 不一定出现在 shader.defines 中；它控制 Pass 是否创建，仍是合法布尔宏。
    for (const technique of A.object(material.effectAsset).techniques as RuntimeObject[] ?? []) for (const pass of technique.passes as RuntimeObject[] ?? []) {
      if (typeof pass.switch === 'string') allowed.set(pass.switch, { type: 'boolean' });
    }
  ...
  async serialized(p: JsonObject): Promise<JsonObject> {
    ...
    try {
      ...
      let propertyPassIndex = p.passIndex === undefined ? undefined : Number(p.passIndex);
      if (p.effectUuid && previous) {
        ...
        const passIndex = p.passIndex === undefined ? undefined : Number(p.passIndex);
        if (passIndex !== undefined && (!Number.isInteger(passIndex) || !this.passes(material)[passIndex])) throw new CocosError('INVALID_ARGUMENT', 'Pass index out of range');
        // 对外索引指当前可见 Pass；Material.copy 的数组却按 technique 原始 Pass 编号，关闭的 switch 不能使两者错位。
        const effectPass = passIndex === undefined ? undefined : Number(this.passes(material)[passIndex]!.passIndex);
        const techniquePasses = (A.object(effect).techniques as RuntimeObject[])[technique]!.passes as RuntimeObject[];
        if (effectPass !== undefined && (!Number.isInteger(effectPass) || !techniquePasses[effectPass])) throw new CocosError('UNSUPPORTED_CAPABILITY', 'Native Effect pass mapping is unavailable');
        const patch = (value: JsonValue): JsonValue => effectPass === undefined ? value : techniquePasses.map((_pass, index) => index === effectPass ? value : {});
        ...
        if (effectPass !== undefined) {
          propertyPassIndex = this.passes(material).findIndex(pass => pass.passIndex === effectPass);
          if (propertyPassIndex < 0) throw new CocosError('INVALID_ARGUMENT', 'Selected Pass was disabled by the requested macros');
        }
      }
      await this.properties(material, Json.object(p.properties ?? {}), propertyPassIndex);
  ...
  async asset(p: JsonObject): Promise<JsonObject> {
    const material = await this.load(Json.string(p.uuid, 'material UUID'), 'Material');
    if (p.action !== 'serialize') return this.describe(material);
    if (!this.environment.serialize) throw new CocosError('UNSUPPORTED_CAPABILITY', 'Editor serializer unavailable');
    const serialized = this.environment.serialize(material);
    return { serialized: Json.value(typeof serialized === 'string' ? JSON.parse(serialized) : serialized), material: this.describe(material) };
  }
