...
import { CocosError, Json, type Capability, type JsonObject, type JsonSchema } from '../../contracts/src/index.js';
...
export class CapabilityCatalog {
  ...
  private readonly deferredValidators = new Map<string, { schema: JsonSchema; validate: ValidateFunction }>();
  ...
  validateDeferred(id: string, params: JsonObject, fields: string[]): Capability {
    ...
    const deferred = [...new Set(fields)].sort();
    // Schema 内容参与键，避免能力定义发生变化后复用旧校验器；字段顺序不影响校验含义。
    const key = Json.canonical({ id, fields: deferred, schema: Json.value(capability.inputSchema) });
    let cached = this.deferredValidators.get(key);
    if (!cached) {
      const schema = structuredClone(capability.inputSchema);
      schema.required = (schema.required as string[] ?? []).filter(field => !deferred.includes(field));
      cached = { schema, validate: this.ajv.compile(schema) };
      if (this.deferredValidators.size >= 128) {
        const oldest = this.deferredValidators.keys().next().value!;
        // 同时释放 Ajv 内部缓存，只有外层 Map 有界并不能限制编译器占用。
        this.ajv.removeSchema(this.deferredValidators.get(oldest)!.schema);
        this.deferredValidators.delete(oldest);
      }
    }
    this.deferredValidators.delete(key); this.deferredValidators.set(key, cached);
    const validate = cached.validate;
