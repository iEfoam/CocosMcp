...
class PanelHarness {
  ...
  private readonly window = parseHTML('<html><body><div id="root"></div></body></html>').window;
  ...
  private clicks(attribute: string): Map<string, () => void> {
    return new Map(Array.from(this.buttons(attribute)).map(([key, button]) => [key, () => button.click()]));
  }
...
test('hidden panel polling pauses and visible polling resumes without duplicate handlers', async () => {
  const harness = new PanelHarness(); let calls = 0;
  const definition = await harness.load(3, async () => { calls++; return snapshot; });
  await harness.ready(definition);
  try {
    harness.root.getClientRects = () => ({ length: 0 }) as DOMRectList;
    for (const poll of harness.polls) poll(); await new Promise(resolve => setImmediate(resolve));
    assert.equal(calls, 1);
    harness.root.getClientRects = () => ({ length: 1 }) as DOMRectList;
    for (const poll of harness.polls) poll(); await new Promise(resolve => setImmediate(resolve));
    assert.equal(calls, 2);
    harness.actions.get('refresh')!.click(); await new Promise(resolve => setImmediate(resolve));
