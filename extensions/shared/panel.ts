...
export function createPanelDefinition(major: 2 | 3) {
  ...
  const render = (): void => {
    ...
    root.innerHTML = `<div class="shell">
      ...
      </main><footer class="footer"><span>CocosMCP ${esc(state.snapshot?.extension?.version ?? '读取中')} <code>${esc(state.snapshot?.extension?.buildId)}</code> <button class="text-button" data-action="extension-update">${state.snapshot?.extension?.updating ? '更新中…' : '更新版本'}</button><small style="display:block">${state.snapshot?.extension?.reloadRequired ? `已安装 ${esc(state.snapshot.extension.installedVersion)} · ${esc(state.snapshot.extension.installedBuildId)}，待重载。` : ''}${esc(state.snapshot?.extension?.message ?? '更新源：本地构建')}</small></span><span>日志时区：${esc(timeZone)}</span></footer>
    </div>`;
    bind();
