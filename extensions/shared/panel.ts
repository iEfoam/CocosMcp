  ...
  const definition = {
    ...
    style: `
      ...
      * { box-sizing: border-box; } #cocos-mcp-control-center { height: 100%; } .shell { height: 100%; display:flex; flex-direction:column; background: radial-gradient(circle at 0% 0%, rgba(79,125,255,.16), transparent 34%), var(--color-normal-fill, #171a21); }
      ...
      .tabs { display:flex; gap:5px; padding:6px 18px 0; border-bottom:1px solid rgba(255,255,255,.07); } .tab { border:0; background:none; color:#7e879b; cursor:pointer; padding:10px 13px 12px; font:600 12px inherit; border-bottom:2px solid transparent; } .tab.selected { color:#fff; border-color:#7188ff; } .tab em { font-style:normal; margin-left:4px; color:#8894b5; font-size:10px; }
      ...
      .select-shell select { appearance:none; -webkit-appearance:none; color:#e4e9f6; background:transparent; border:0; border-radius:inherit; font:500 11px -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif; padding:0 32px 0 30px; min-height:30px; min-width:140px; cursor:pointer; outline:none; color-scheme:dark; }
      .select-shell option { color:#e4e9f6; background:#262c3a; } .select-chevron { position:absolute; right:11px; top:4px; color:#a7b7d9; font-size:16px; pointer-events:none; }
      .level-dot { position:absolute; left:12px; width:6px; height:6px; border-radius:50%; background:#a5b4d6; pointer-events:none; } .level-dot.error { background:#ff95a2; } .level-dot.warn { background:#efc77c; } .level-dot.info { background:#8eb6ff; }
      ...
      .log-heading-actions select { min-height:28px; min-width:126px; font-size:10px; }
