...
import { SceneResponse } from './scene-response.js';
...
class CreatorHost implements EditorPort {
  ...
  async scene(method: string, ...args: unknown[]): Promise<unknown> { return SceneResponse.unwrap(await this.request('scene', 'execute-scene-script', { name: this.extensionName, method: 'dispatch', args: [method, args] })); }
...
export const methods = {
  ...
  panelState: (cursor?: string): Promise<PanelState> => lifecycle.panelState(cursor),
