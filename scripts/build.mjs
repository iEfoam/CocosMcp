...
await build({ entryPoints: ['apps/server/src/cli.ts'], outfile: join(output, 'server/cli.mjs'), bundle: true, platform: 'node', target: 'node24', format: 'esm',
...
for (const major of [2, 3]) {
  ...
  if (major === 3) await build({ entryPoints: ['packages/runtime3-bridge/src/bootstrap.ts'], outfile: join(extensionRoot, 'dist/runtime.js'), bundle: true,
...
await build({ entryPoints: ['packages/runtime3-bridge/src/bootstrap.ts'], outfile: join(output, 'runtime/cocos-mcp.js'), bundle: true,
  platform: 'browser', target: 'es2018', format: 'iife', globalName: 'CocosMCPRuntime', sourcemap: true });
await build({ entryPoints: ['packages/runtime3-bridge/src/bootstrap.ts'], outfile: join(output, 'runtime/cocos-mcp.mjs'), bundle: true,
