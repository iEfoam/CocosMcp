...
for(const major of [2,3]) {
 ...
 for(const path of ['package.json','LICENSE',`dist/main.${ext}`,`dist/scene.${ext}`,`dist/panel.${ext}`,'dist/service.mjs','dist/update.mjs', ...(major === 3 ? ['dist/runtime.js'] : [])]) rows.push({path,content:(await readFile(join(root,path))).toString('base64')});
